...
from mesh.receiver_mesh import MeshNetwork, DetectionEvent
from mesh.triangulation import Triangulator
...
class GhostNet:
    ...
    def __init__(self, config_path: str):
        ...
        # Store recent detections from mesh network
    ...
    def _load_config(self, config_path: str) -> Dict:
        ...
        try:
            ...
            logger.error("Failed to open config file '%s': %s", config_path, str(e))
            sys.exit(1)
        except json.JSONDecodeError as e:
            logger.error("Failed to parse config file '%s': %s", config_path, str(e))
            sys.exit(1)
        except OSError as e:
            logger.error("OS error while loading config file '%s': %s", config_path, str(e))
            sys.exit(1)
    ...
    def _initialize_sdr_devices(self):
        ...
        for device_id, device_config in self.config['sdr_devices'].items():
            ...
            if not self.sdr_manager.add_device(
                ...
                logger.error("Failed to initialize SDR device: %s", device_id)
    ...
    def _processing_loop(self):
        ...
        while self.running:
            try:
                ...
                for _, device in self.sdr_manager.devices.items():
                    ...
                    if samples is not None:
                        ...
                        # Store local detection for triangulation
        ...
        while self.running:
            try:
                ...
                if location:
                    ...
                    # Log location
                    logger.info(
                        "Target located at %s (confidence: %.2f, classification: %s)",
                        location.position,
                        location.confidence,
    ...
    def _store_detection(self, detection: DetectionEvent):
        """Store a detection event for triangulation."""
        ...
        # Clean up old detections (older than timeout)
        current_time = time.time()
        self.recent_detections = [
            d for d in self.recent_detections
            if current_time - d['timestamp'] < self.detection_timeout
        ]

    def _get_recent_detections(self) -> List[Dict]:
        """Get recent detections from other nodes."""
        # Get detections from mesh network
        mesh_detections = []
        # Check if mesh network has received detections
        try:
            # Access the detection queue from mesh network
            mesh_queue = getattr(self.mesh_network, 'detection_queue', None)
            if mesh_queue and not mesh_queue.empty():
                # Get all available detections from mesh network
                while not mesh_queue.empty():
                    try:
                        detection = mesh_queue.get_nowait()
                        # Only include detections from other nodes
                        if detection.node_id != self.config['mesh']['node_id']:
                            mesh_detections.append({
                                'position': detection.position or (0, 0, 0),
                                'signal_features': detection.signal_features,
                                'timestamp': detection.timestamp,
                                'classification': detection.classification,
                                'node_id': detection.node_id
            ...
            logger.warning("Error accessing mesh network detections: %s", str(e))
        return self.recent_detections + mesh_detections
